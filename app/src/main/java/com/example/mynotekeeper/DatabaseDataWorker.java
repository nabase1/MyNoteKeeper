...
public class DatabaseDataWorker {
    ...
    private void insertCourse(String courseId, String title) {
        ...
        values.put(MyNoteDbContract.CourseEntryInfo.COLUMN_COURSE_ID, courseId);
        values.put(MyNoteDbContract.CourseEntryInfo.COLUMN_COURSE_TITLE, title);

        long newRowId = mDb.insert(MyNoteDbContract.CourseEntryInfo.TABLE_NAME, null, values);
    }

     private void insertNote(String courseId, String title, String text) {
        ContentValues values = new ContentValues();
        values.put(MyNoteDbContract.NoteInfoEntry.COLUMN_COURSE_ID, courseId);
        values.put(MyNoteDbContract.NoteInfoEntry.COLUMN_NOTE_TITLE, title);
        values.put(MyNoteDbContract.NoteInfoEntry.COLUMN_NOTE, text);

        long newRowId = mDb.insert(MyNoteDbContract.NoteInfoEntry.TABLE_NAME, null, values);
    }
